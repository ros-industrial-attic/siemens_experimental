        ...
        PNIO_IOXS iops)
{
...
#ifdef DEBUG
  ROS_INFO("PNIO_cbf_data_read(..., len=%u, Iops=%u) for devHandle 0x%x, slot %u, subslot %u",
  ...
  else if(buffer_length <= (PNIO_UINT32)NUMOF_BYTES_PER_SUBSLOT)
  {
    ...
    printf("OutData: [slot %d]", slot_num);
    for(i = 0; i < buffer_length; i++)
    {
      if(i % 16 == 0 && i!=0)
      printf("\n");

      printf(" 0x%02x", CallbackHandler->out_data_[slot_num][subslot_num][i]);
    }
...
#if DEBUG
  ROS_INFO("PNIO_cbf_data_write(len = %u, Iocs = %u devHandle = %u, slot = %u, subslot = %u",
...
#ifdef DEBUG
  ROS_INFO("WRITE RECORD Request, Api=%lu Slot=%lu Subslot=%lu Index=%lu, Length=%lu, Sequence_nr=%lu",
    (unsigned long)api,
...
#ifdef DEBUG
  ROS_INFO("READ_RECORD Request, Api=%lu Slot=%lu Subslot=%lu Index=%lu, Length=%lu, Sequence_nr=%lu",
    (unsigned long)api,
    (unsigned long)p_addr->u.Geo.Slot,
    (unsigned long)p_addr->u.Geo.Subslot,
    (unsigned long)record_index,
...
void PNIOCbfForCheckIndication(
        PNIO_UINT32 dev_handle,
        PNIO_UINT32 api,
        PNIO_UINT16 ar_number,
...
void PNIOCbfForArCheckIndication(
        PNIO_UINT32 dev_handle,
        PNIO_UINT32 host_ip,
        PNIO_UINT16 ar_type,
...
void PNIOCbfForArInfoIndication(
        PNIO_UINT32 dev_handle,
        PNIO_UINT16 ar_number,
        PNIO_UINT16 session_key,
        PNIO_AR_TYPE *p_ar)
...
void PNIOCbfForArIndataIndication(
        PNIO_UINT32     dev_handle,
        PNIO_UINT16     ar_number,
        PNIO_UINT16     session_key)
{
...
void PNIOCbfForArAbortIndication(
        PNIO_UINT32 dev_handle,
        PNIO_UINT16 ar_number,
        PNIO_UINT16 session_key,
...
void PNIOCbfForArOfflineIndication(
        PNIO_UINT32 dev_handle,
        PNIO_UINT16 ar_number,
        PNIO_UINT16 session_key,
        PNIO_AR_REASON reason_code)
...
void PNIOCbfForPrmEndIndication(
        PNIO_UINT32 dev_handle,
        PNIO_UINT16 ar_number,
        PNIO_UINT16 session_key,
        PNIO_UINT32 api,
...
void PNIOCbfForCpStopRequest(
        PNIO_UINT32 dev_handle)
{
  ROS_INFO_STREAM("PNIOCbfCpStopReq called");
}

void PNIOCbfForDeviceStopped(
        PNIO_UINT32 dev_handle,
...
void PNIOCbfForApduStatusIndication(
        PNIO_UINT32 dev_handle,
        PNIO_UINT16 ar_number,
        PNIO_UINT16 session_key,
